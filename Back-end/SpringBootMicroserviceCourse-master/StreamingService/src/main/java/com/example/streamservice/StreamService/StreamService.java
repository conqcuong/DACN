...
public class StreamService {
	...
	public void recordHLS(String inputUrl, String outputFilePath) throws IOException {
		try {
			...
			String ffmpegPath = "D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\ffmpeg.exe";

			String command = "D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\ffmpeg.exe -i http://localhost:8080/hls/locnguyen1234.m3u8 -c copy D:\\SpringBootMicroservice-master\\ffmpeg-6.1-essentials_build\\bin\\record\\output1.mp4";
	...
	public Mono<Stream> createStream(Stream stream, String inputUrl, String outputFilePath) throws IOException {
		...
		String ffmpegPath = "D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\ffmpeg.exe";
		String command = "D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\ffmpeg.exe -i http://localhost:8080/hls/"
				+ stream.getSecretkey() + ".m3u8"
				+ " -c copy -f hls -hls_time 10 -hls_list_size 6 -hls_flags delete_segments D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\record\\"
				+ stream.getSecretkey() + ".mp4";
		FFmpeg ffmpeg = new FFmpeg("D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\ffmpeg.exe");
		FFmpegBuilder job = ffmpeg.builder()
		        .setInput("http://localhost:8080/hls/" + stream.getSecretkey() + ".m3u8")
		        .addOutput("D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\record\\"+stream.getSecretkey()+".mp4")
		        .done();
		new FFmpegExecutor(ffmpeg).createJob(job).run();
		
		File livestreamFile = new File("D:\\DACN\\Back-end\\SpringBootMicroserviceCourse-master\\ffmpeg-6.1-essentials_build\\bin\\record\\"+ stream.getSecretkey() + ".mp4");
