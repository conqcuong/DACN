...
export const EditCoursePage = () => {
    ...
    const courses = useSelector((state)=> state.course.currentCourse); 
    // console.log(courses);
    // const productName = courses[0].nameproduct;
    useEffect(() => {
        if (courses && courses[0]) {
            setName(courses[0].nameproduct);
            setTitle(courses[0].description);
            setPrice(courses[0].price);
            setImage(courses[0].apiimage);
            // setFilePath("courses[0].apiimage");
        }
      }, [courses]);
    ...
    useEffect(() => {
        const fetchData = async() =>{
            try{
                await getOneCourse(dispatch, courseId);
            }catch(err){
                console.log(err);
            }
        }
    ...
    const updateCourse = async (formData, courseId, dispatch, navigate, showToast) => {
        try {
            const response = await axios.put(`http://localhost:9000/product/Edit/${courseId}`, formData);
    ...
    const handleCourse = async (e) => {
        try {
            ...
            await updateCourse(formData, courseId, dispatch, navigate, showToast);
        ...
        <div className='flex min-h-screen '>
            ...
            <div className='ml-[250px] mr-[150px] max-w-full flex-1 lg:w-[calc(100%_-_96px)] md:px-8 sm:px-4 mt-10'>
                <section className='w-full max-w-[1920px] p-0 mx-auto'>
                    <div>
                        <h1 className='text-[28px] font-bold'>Sửa khóa học</h1>
                        <form className='flex justify-between gap-10 ' onSubmit={handleCourse}>
                            <div className='w-[60%]'>
                                ...
                                <div className='flex border border-solid border-gray-300 h-11 overflow-hidden'>
                                    <input className='border-none flex-1 px-5 py-3 outline-none' type="text" placeholder="Tên khóa học" value={name} onChange={(e)=>setName(e.target.value)}/>
                                </div>
                                <div className='flex justify-between'>
                                ...
                                <div className='flex border border-solid border-gray-300'>
                                    <textarea rows='4' className='border-none flex-1 px-5 py-3 outline-none h-24 w-full' type="text" placeholder="Mô tả" value={title} onChange={(e)=>setTitle(e.target.value)} />
                                </div>
                                <div className='flex justify-between'>
                                    <label className='block text-14px font-semibold my-2.5 ml-2 text-black'>Giá:</label>
                                </div>
                                <div className='flex border border-solid border-gray-300 h-11 overflow-hidden'>
                                    <input className='border-none flex-1 px-5 py-3 outline-none' type="text" placeholder="Giá" value={price} onChange={(e)=>setPrice(e.target.value)}/>
                                </div> 
                                <div className='flex justify-between'>
