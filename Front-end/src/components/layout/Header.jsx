...
export const Header = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:8000/v1/course/")
        .then((res) => res.json())
        .then((data) => {
            setFilterData(data);
        })
  ...
  return (
    <>
                ...
                <div className="z-9999 inset-0 absolute transform translate-x-[0] translate-y-[50px]">
                    <ul>
                        <div className="search__result">
                            ...
                            {data.map((d, i) => (
                                <Link key={i} to={`/course/details/${d._id}`} className="flex items-center py-1.5">
                                    <div className="text-[3.6px] rounded-full">
                                        <img className="w-[32.4px] h-[32.4px] object-cover rounded-full" src={d.cover}/>
                                    </div>
        ...
        <div className="navbar__actions">
            ...
            <>
                <button className="text-black font-semibold mr-8" onClick={handleLoginClick}>Đăng nhập</button>
                <button className="font-semibold text-white bg-gradient-to-r from-orange-500 to-[#ff5117] rounded-[99px] py-[9px] px-5 hover:opacity-90" onClick={handleRegisterClick}> Đăng ký</button>
            </>
        )}
        </div>
        </div>
